...
<?php 
		...
		<div class="InnerPortfolio"><div id="afp-container">';
		...
		//Start Echo Categories
                ...
                $page_id = $post_obj->ID;
                if($page_id == '16'){
        $output.='<div id="afp-container-right"><ul id="afp-filter">
        <li id="cat-all" class="afp-active-cat"><a href="#" class="All">' . __('All', 'awesome-filterable-portfolio') . '</a></li>';         
        foreach ( $cats as $cat ){
            	$output.='<li id="cat-'.$cat->cat_id.'"><a href="#" class="' . ereg_replace("[^A-Za-z0-9]", "", $cat->cat_name) . '">' . $cat->cat_name . '</a></li>';
        }
        $output.='</ul></div>';
                }
		//End Echo Categories
        ...
        $output.='<div id="afp-container-left"><ul class="afp-items view">';
        $k = 1;
//        foreach ($items as $item ){
...
//		}
        foreach ($items as $item ){
            	$output.='<li class="view-tenth afp-single-item" data-id="id-' . $k . '" data-type="' . ereg_replace("[^A-Za-z0-9]", "", $item->item_category) .'"><img alt="" class="img-link-initial" src="' . $item->item_thumbnail . '">
                <div class="mask">                
                <a target="_' . $afpOptions['project_link'] . '" href="' . $item->item_link . '"><div class="link"><i class="fa fa-link"></i></div></a>';
                    $output.='<a class="colorbox" title="' . $item->item_description . '" href="' . $item->item_image . '"><div class="search"><i class="fa fa-search"></i></div></a>';
//                   if($item->item_link != null) { $output.='<li><a target="_' . $afpOptions['project_link'] . '" href="' . $item->item_link . '">' . __('Project Link', 'awesome-filterable-portfolio') . '</a></li>'; }			
                $output.='</div>
            </li>';
